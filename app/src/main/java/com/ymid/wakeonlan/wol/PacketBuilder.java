...
class PacketBuilder {
    ...
    static DatagramPacket buildMagicPacket(String broadcastAddress, String macAddress, int port,
        ...
        InetAddress address = resolveAddress(broadcastAddress);
    ...
    static boolean isIpv6(String address) {
        return address != null && IPV6_PATTERN.matcher(address).matches();
    }
    ...
    private static byte[] getIpBytes(String ipString) throws IllegalArgumentException {
        ...
        if (ipOctets.length != 4) throw new IllegalArgumentException("Invalid IP address.");
        try {
            for (int i = 0; i < 4; i++) {
